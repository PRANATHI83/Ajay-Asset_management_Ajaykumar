...
CREATE TABLE IF NOT EXISTS asset_requests (
    ...
    CONSTRAINT valid_email CHECK (email ~* '^[a-zA-Z0-9][a-zA-Z0-9\\-_\\.]{1,28}[a-zA-Z0-9]@gmail\\.com$'),
    CONSTRAINT valid_employee_id CHECK (employee_id ~ '^ATS0[0-9]{3}$' AND employee_id <> 'ATS0000')
);

-- Create a UNIQUE constraint to support ON CONFLICT DO NOTHING
-- (Prevents duplicate asset requests for the same employee on the same date)
